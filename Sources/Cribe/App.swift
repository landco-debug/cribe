...
final class AppCore: ObservableObject {
    ...
    /// Модель поднимается в память сразу после запуска, а не при первом ⌘.
    ///
    /// Загрузка занимает секунды, и раньше человек платил их ожиданием ровно тогда, когда
    /// уже начал говорить: надпись «Готовлю модель…» появлялась на самой первой диктовке.
    /// Приложение живёт в строке состояния и стартует вместе с системой — время на прогрев
