...
## 0. Глобальные правила: говорящие имена и без лишней вложенной упаковки
...
### Без лишних вложенных архивов

**Не создавать «архив внутри архива» без технической необходимости.** Пользователь должен получать максимально прямой результат: один файл, который можно сразу распаковать/установить, а не проходить лишнюю цепочку ZIP → ZIP → app.

Правило для всех подпроектов и веток:

- если транспорт уже автоматически упаковывает выдаваемый файл, не добавлять ещё один архивный слой просто «для порядка»;
- если GitHub Actions Artifact содержит ровно один готовый ZIP/DMG/PKG, при непосредственной выдаче пользователю нужно снять внешний служебный слой Artifact и выдать сам полезный файл;
- не оборачивать ZIP в другой ZIP при ручной переименовке или передаче между чатами/инструментами;
- если вложенная упаковка **технически необходима** (например, чтобы сохранить executable bits, extended attributes, code signature или структуру macOS `.app` при передаче через систему, которая сама меняет права файлов), это допустимое исключение, но оно должно быть осознанным и зафиксированным;
- даже при таком исключении пользователю по возможности выдаётся уже извлечённый внутренний payload, чтобы от него не требовалось распаковывать два слоя.

Перед выдачей сборки проверять не только имя, но и **глубину упаковки**. Лишний архивный слой считается дефектом handoff, а не косметикой.
